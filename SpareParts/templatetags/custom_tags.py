...
def order_op4(order_id):
    return SparePartsOrderOperations.objects.filter(order_number__id=order_id, operation_type=4)

@register.simple_tag(name='order_op2')
def order_op2(order_id):
    return SparePartsOrderOperations.objects.filter(order_number__id=order_id, operation_type=2)
